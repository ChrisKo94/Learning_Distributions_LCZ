...
def evaluation(res_ckpt_filepath):
    ...
    # Cap preds_id and preds_ood at maximum of 10 to avoid numerical instability
    preds_id = np.minimum(preds_id, 10)
    preds_ood = np.minimum(preds_ood, 10)
    ...
    if mode == "one-hot" or mode == "distributional" or mode == "sampled_one-hot":
        ...
        auroc_distr_unc = "NA"
        aupr_distr_unc = "NA"
    # for numerical stability, add small epsilons to probs_id and probs_ood
    epsilons = 1e-10
    probs_id = probs_id + epsilons
    probs_ood = probs_ood + epsilons
    ...
    if mode == "dirichlet" or mode == "Dirichlet_embedding" or mode == "MSE_embedding" or mode == "Mahala_embedding":
        dsm_id = 9 / (np.sum(alphas_id, axis=-1, keepdims=True) + 9)
        dsm_ood = 9 / (np.sum(alphas_ood, axis=-1, keepdims=True) + 9)
        ...
        distr_unc_id = -np.sum(probs_id*(np.log(probs_id)-psi(alphas_id+1)+psi(np.sum(alphas_id+1, axis=-1, keepdims=True))), axis=-1)
        distr_unc_ood = -np.sum(probs_ood*(np.log(probs_ood)-psi(alphas_ood+1)+psi(np.sum(alphas_ood+1, axis=-1, keepdims=True))), axis=-1)
        ...
        aupr_distr_unc = auc(*precision_recall_curve(targets, np.concatenate((distr_unc_id, distr_unc_ood), axis=0))[-2:-4:-1])
    # Store results
    res = {
        'auroc_dsm': auroc_dsm,
        'auroc_msp': auroc_msp,
        'auroc_pred_ent': auroc_pred_ent,
        'auroc_exp_ent': auroc_exp_ent,
        'auroc_evidence': auroc_evidence,
        'auroc_distr_unc': auroc_distr_unc,
        'aupr_dsm': aupr_dsm,
        'aupr_msp': aupr_msp,
        'aupr_pred_ent': aupr_pred_ent,
        'aupr_exp_ent': aupr_exp_ent,
        'aupr_evidence': aupr_evidence,
        'aupr_distr_unc': aupr_distr_unc
    }
...
if __name__ == "__main__":
    for mode in ["one-hot", "distributional", "sampled_one-hot", "dirichlet","Dirichlet_embedding","MSE_embedding", "Mahala_embedding"]: #
        for seed in range(1):
            ...
            res_ckpt_filepath = Path(path, "results", f"Sen2LCZ_bs_{batchSize}_lr_{lrate}_seed_{seed}_weights_best_{mode}_urban_as_id_20.hdf5")
            # Start evaluation
            res = evaluation(res_ckpt_filepath)
            # Store results in overall results matrix
            results = results.append(res, ignore_index=True)
# Write ALL results to disk
results.to_csv(Path(path,"results","0.0002_results_veg_as_ood_20.csv"))