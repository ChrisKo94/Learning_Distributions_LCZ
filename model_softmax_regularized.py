...
def sen2LCZ_drop_core(inputs, num_classes=17, bn=1, depth=5, dim=16, dropRate=0.1, fusion=0, l2_reg = 0.01):
    ...
    '32*32'
    conv0 = Conv2D(dim, (3, 3), padding='same', kernel_initializer = 'he_normal',
    ...
    for i in np.arange(lay_per_block-1):
        ...
        conv0 = Conv2D(dim, (3, 3), padding='same', kernel_initializer = 'he_normal',
                        kernel_regularizer=l2(l2_reg), bias_regularizer=l2(l2_reg))(conv0)
        if bn==1:
            print('with BN')
            conv0 = BatchNormalization(axis=-1)(conv0)
        conv0 = Activation('relu')(conv0)
    ...
    '16*16'
    dim=dim*inc_rate
    conv1 = Conv2D(dim, (3, 3), padding='same', kernel_initializer = 'he_normal',
                   kernel_regularizer=l2(l2_reg), bias_regularizer=l2(l2_reg))(merge0)
    if bn==1:
        print('with BN')
    ...
    for i in np.arange(lay_per_block-1):
        print(str(i) +'in' +str(lay_per_block-1), '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        conv1 = Conv2D(dim, (3, 3), padding='same', kernel_initializer = 'he_normal',
                       kernel_regularizer=l2(l2_reg), bias_regularizer=l2(l2_reg))(conv1)
        if bn==1:
            print('with BN')
            conv1 = BatchNormalization(axis=-1)(conv1)
        conv1 = Activation('relu')(conv1)
    ...
    '8*8'
    dim=dim*inc_rate
    conv2 = Conv2D(dim, (3, 3), padding='same', kernel_initializer = 'he_normal',
                   kernel_regularizer=l2(l2_reg), bias_regularizer=l2(l2_reg))(merge1)
    if bn==1:
        print('with BN')
    ...
    for i in np.arange(lay_per_block-1):
        print(str(i) +'in' +str(lay_per_block-1), '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        conv2 = Conv2D(dim, (3, 3), padding='same', kernel_initializer = 'he_normal',
                       kernel_regularizer=l2(l2_reg), bias_regularizer=l2(l2_reg))(conv2)
        if bn==1:
            print('with BN')
            conv2 = BatchNormalization(axis=-1)(conv2)
        conv2 = Activation('relu')(conv2)
    ...
    '4*4'
    dim=dim*inc_rate
    conv3 = Conv2D(dim, (3, 3), padding='same', kernel_initializer = 'he_normal',
                   kernel_regularizer=l2(l2_reg), bias_regularizer=l2(l2_reg))(merge2)
    if bn==1:
        print('with BN')
    ...
    for i in np.arange(lay_per_block-1):
        print(str(i) +'in' +str(lay_per_block-1), '!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
        conv3 = Conv2D(dim, (3, 3), padding='same', kernel_initializer = 'he_normal',
                       kernel_regularizer=l2(l2_reg), bias_regularizer=l2(l2_reg))(conv3)
        if bn==1:
            print('with BN')
            conv3 = BatchNormalization(axis=-1)(conv3)
        conv3 = Activation('relu')(conv3)
...
def sen2LCZ_drop(input_shape=(32,32,10), num_classes=17, bn=1, depth=5, dim=16, dropRate=0.1, fusion=0, l2_reg = 0.01):
    """
    ...
    o=sen2LCZ_drop_core(inputs, num_classes=num_classes, bn=bn, depth=depth, dim=dim, dropRate=dropRate, fusion=fusion, l2_reg = l2_reg)
